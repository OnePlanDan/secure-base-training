"""
Random-subsample baseline: 2,502 random HH-RLHF pairs (seed=123).
Controls for whether SBT curation matters or HH-RLHF is simply redundant.
"""
import sys
sys.path.insert(0, str(__import__("pathlib").Path(__file__).parent.parent))

from datasets import load_dataset
from config import RANDOM_DPO_DATA

NUM_EXAMPLES = 2_502
SEED = 123  # Different from standard (42) to get a distinct subsample
...
def build():
    ...
    ds = ds.shuffle(seed=SEED).select(range(min(NUM_EXAMPLES * 2, len(ds))))
    ...
    print(f"Built {len(result)} random-subsample DPO pairs (seed={SEED})")
    result.save_to_disk(str(RANDOM_DPO_DATA))
    print(f"Saved to {RANDOM_DPO_DATA}")
